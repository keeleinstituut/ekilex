...
$(document).on("click", "#homonymListToggleButton", function() {
	...
	const isExpanded = homonymList.hasClass("expand");
	$(this).attr("aria-expanded", !isExpanded);
	homonymList.toggleClass("expand");
});

$(document).on("click", "#clear-search-btn", function(e) {
	const searchInput = $("input[name='searchWordValue']");
...
$(document).on("click", "eki-link", function(e) {
	...
	$.get(searchLinkUrlWithParams).done(function(data) {
		if (data) {
			...
			searchForm.find("input[name='searchWordValue']").val(data.wordValue);
			searchForm
				.find("input[name='selectedWordHomonymNr']")
				.val(data.homonymNr);
...
$(document).on("shown.bs.modal", "[id^='morpho-modal-']", function() {
	...
	var morphoUrlWithParams =
		morphoUrl + "/" + paradigmId + "/" + currentWordLang;
...
function searchWordAutocompleteHeight() {
	if ($(".keyboard-search").hasClass("lang-open")) {
		...
		if (search.length && virtualKeyboard.length) {
			...
			let calculateNewHeight =
				documentHeight -
				heightFromTopToSearchInput -
				searchInputHeight -
				heightKeyboard;
			let maxAutocompleHeight = calculateNewHeight
				? calculateNewHeight + "px"
				: "";
...
function searchWordAutocomplete() {
	...
	var searchWordAutocompleteConfig = {
		...
		minLength: 3,
		create: function() {
			$(this).data("uiAutocomplete")._renderMenu =
				searchWordAutocompleteMenuRenderer;
		},
		select: function(event, ui) {
			if (ui.item) {
				$("input[name='searchWordValue']").val(ui.item.value);
	...
	$(document).on("change input", "input[name='searchWordValue']", function() {
		if ($(".keyboard-search").hasClass("lang-open")) {
			// virtual keyboard enabled
			if ($("#KioskBoard-VirtualKeyboard").length) {
				// only run when virtual keyboard exist
				$(this)
...
function clickSearchIfInputExists() {
	var tempSearchWord = $("input[name='searchWordValue']").val();
	if (tempSearchWord) {
		$("#search-btn").trigger("click");
	}
}
...
function initDatasetDropdown() {
	...
	$(document).on(
		"click",
		"#dataset-filter-wrapper .search-filter__menu",
		function(e) {
			e.stopPropagation();
		},
	);
	...
	$(document).on(
		"change",
		"#dataset-filter-wrapper .search-filter__menu input",
		function(e) {
			e.stopPropagation();
			if (!menuContainer) {
			...
			if (e.target?.dataset?.filterCode === datasetAllCode) {
				...
				inputAll.checked = inputs.every(
					(input) => input === inputAll || input.checked,
				);
			}
		},
	);
...
function updateLangFilterStates(codes, checkboxes) {
	...
	if (countElement) {
		countElement.innerText = !codes.startsWith(langAllCode)
			? checkboxes.length
			: "";
	}
...
function initLangDropdown() {
	...
	$(document).on(
		"click",
		"#lang-filter-wrapper .search-filter__menu",
		function(e) {
			e.stopPropagation();
		},
	);
	...
	$(document).on(
		"change",
		"#lang-filter-wrapper .search-filter__menu input",
		function(e) {
			e.stopPropagation();
			if (!menuContainer) {
				menuContainer = document.getElementById(
	...
	$(document).on("click", "#lang-filter-tags button", function(e) {
		const targetCheckbox = menuContainer.querySelector(
			`input[data-filter-code=${e.currentTarget.dataset?.filterCode}]`,
		);
...
function initWordDetails() {
	...
	addCollapseAriaAttributes();
}

function addCollapseAriaAttributes() {
	const collapseButtons = document.querySelectorAll(
		"button[data-toggle='collapse'][data-dynamic-id-collapse-container]",
	);
	collapseButtons.forEach((button) => {
		const containerId = button.dataset.dynamicIdCollapseContainer;
		const container = document.getElementById(containerId);
		if (!container) {
			return;
		}
		const targetElements = container.querySelectorAll(
			"[data-dynamic-id-collapse-target]",
		);
		const targetIds = [];
		targetElements.forEach((element) => {
			...
			if (element.id) {
				...
				element.id = `${containerId}-collapse-target-${crypto.randomUUID()}`;
				targetIds.push(element.id);
			}
		});
