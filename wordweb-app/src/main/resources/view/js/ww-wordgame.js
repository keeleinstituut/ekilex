			...
			if ((audioValue != null && audioValue != "") || (item.getAttribute('data-words') != null && item.getAttribute('data-words') != "")) {
				item.addEventListener('click', (e) => {
					...
					if (item.getAttribute('data-words')) {
						...
						var definedAudioUrl = item.getAttribute('data-audio-url');
						if (definedAudioUrl) {
							playAudio(definedAudioUrl, function() {
							});
							return;
						}

						var data = {
							'text': item.getAttribute('data-words'),
							'serviceId': 'speech'
						};

						$.post(applicationUrl + 'audio-link', data).done(function(providedAudioUrl) {
							item.setAttribute('data-audio-url', providedAudioUrl);
							playAudio(providedAudioUrl, function() {
							});
						}).fail(function() {
