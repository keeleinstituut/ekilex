/*
 * This file is generated by jOOQ.
 */
package eki.wordweb.data.db.tables.records;


import eki.wordweb.data.db.tables.MviewWwMeaning;

import java.time.LocalDateTime;

import org.jooq.Field;
import org.jooq.JSON;
import org.jooq.Record10;
import org.jooq.Row10;
import org.jooq.impl.TableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class MviewWwMeaningRecord extends TableRecordImpl<MviewWwMeaningRecord> implements Record10<Long, LocalDateTime, LocalDateTime, JSON, JSON, JSON, String[], String[], JSON, JSON> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>public.mview_ww_meaning.meaning_id</code>.
     */
    public void setMeaningId(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.meaning_id</code>.
     */
    public Long getMeaningId() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.manual_event_on</code>.
     */
    public void setManualEventOn(LocalDateTime value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.manual_event_on</code>.
     */
    public LocalDateTime getManualEventOn() {
        return (LocalDateTime) get(1);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.last_approve_or_edit_event_on</code>.
     */
    public void setLastApproveOrEditEventOn(LocalDateTime value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.last_approve_or_edit_event_on</code>.
     */
    public LocalDateTime getLastApproveOrEditEventOn() {
        return (LocalDateTime) get(2);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.domain_codes</code>.
     */
    public void setDomainCodes(JSON value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.domain_codes</code>.
     */
    public JSON getDomainCodes() {
        return (JSON) get(3);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.meaning_images</code>.
     */
    public void setMeaningImages(JSON value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.meaning_images</code>.
     */
    public JSON getMeaningImages() {
        return (JSON) get(4);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.media_files</code>.
     */
    public void setMediaFiles(JSON value) {
        set(5, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.media_files</code>.
     */
    public JSON getMediaFiles() {
        return (JSON) get(5);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.semantic_types</code>.
     */
    public void setSemanticTypes(String[] value) {
        set(6, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.semantic_types</code>.
     */
    public String[] getSemanticTypes() {
        return (String[]) get(6);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.learner_comments</code>.
     */
    public void setLearnerComments(String[] value) {
        set(7, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.learner_comments</code>.
     */
    public String[] getLearnerComments() {
        return (String[]) get(7);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.notes</code>.
     */
    public void setNotes(JSON value) {
        set(8, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.notes</code>.
     */
    public JSON getNotes() {
        return (JSON) get(8);
    }

    /**
     * Setter for <code>public.mview_ww_meaning.definitions</code>.
     */
    public void setDefinitions(JSON value) {
        set(9, value);
    }

    /**
     * Getter for <code>public.mview_ww_meaning.definitions</code>.
     */
    public JSON getDefinitions() {
        return (JSON) get(9);
    }

    // -------------------------------------------------------------------------
    // Record10 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row10<Long, LocalDateTime, LocalDateTime, JSON, JSON, JSON, String[], String[], JSON, JSON> fieldsRow() {
        return (Row10) super.fieldsRow();
    }

    @Override
    public Row10<Long, LocalDateTime, LocalDateTime, JSON, JSON, JSON, String[], String[], JSON, JSON> valuesRow() {
        return (Row10) super.valuesRow();
    }

    @Override
    public Field<Long> field1() {
        return MviewWwMeaning.MVIEW_WW_MEANING.MEANING_ID;
    }

    @Override
    public Field<LocalDateTime> field2() {
        return MviewWwMeaning.MVIEW_WW_MEANING.MANUAL_EVENT_ON;
    }

    @Override
    public Field<LocalDateTime> field3() {
        return MviewWwMeaning.MVIEW_WW_MEANING.LAST_APPROVE_OR_EDIT_EVENT_ON;
    }

    @Override
    public Field<JSON> field4() {
        return MviewWwMeaning.MVIEW_WW_MEANING.DOMAIN_CODES;
    }

    @Override
    public Field<JSON> field5() {
        return MviewWwMeaning.MVIEW_WW_MEANING.MEANING_IMAGES;
    }

    @Override
    public Field<JSON> field6() {
        return MviewWwMeaning.MVIEW_WW_MEANING.MEDIA_FILES;
    }

    @Override
    public Field<String[]> field7() {
        return MviewWwMeaning.MVIEW_WW_MEANING.SEMANTIC_TYPES;
    }

    @Override
    public Field<String[]> field8() {
        return MviewWwMeaning.MVIEW_WW_MEANING.LEARNER_COMMENTS;
    }

    @Override
    public Field<JSON> field9() {
        return MviewWwMeaning.MVIEW_WW_MEANING.NOTES;
    }

    @Override
    public Field<JSON> field10() {
        return MviewWwMeaning.MVIEW_WW_MEANING.DEFINITIONS;
    }

    @Override
    public Long component1() {
        return getMeaningId();
    }

    @Override
    public LocalDateTime component2() {
        return getManualEventOn();
    }

    @Override
    public LocalDateTime component3() {
        return getLastApproveOrEditEventOn();
    }

    @Override
    public JSON component4() {
        return getDomainCodes();
    }

    @Override
    public JSON component5() {
        return getMeaningImages();
    }

    @Override
    public JSON component6() {
        return getMediaFiles();
    }

    @Override
    public String[] component7() {
        return getSemanticTypes();
    }

    @Override
    public String[] component8() {
        return getLearnerComments();
    }

    @Override
    public JSON component9() {
        return getNotes();
    }

    @Override
    public JSON component10() {
        return getDefinitions();
    }

    @Override
    public Long value1() {
        return getMeaningId();
    }

    @Override
    public LocalDateTime value2() {
        return getManualEventOn();
    }

    @Override
    public LocalDateTime value3() {
        return getLastApproveOrEditEventOn();
    }

    @Override
    public JSON value4() {
        return getDomainCodes();
    }

    @Override
    public JSON value5() {
        return getMeaningImages();
    }

    @Override
    public JSON value6() {
        return getMediaFiles();
    }

    @Override
    public String[] value7() {
        return getSemanticTypes();
    }

    @Override
    public String[] value8() {
        return getLearnerComments();
    }

    @Override
    public JSON value9() {
        return getNotes();
    }

    @Override
    public JSON value10() {
        return getDefinitions();
    }

    @Override
    public MviewWwMeaningRecord value1(Long value) {
        setMeaningId(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value2(LocalDateTime value) {
        setManualEventOn(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value3(LocalDateTime value) {
        setLastApproveOrEditEventOn(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value4(JSON value) {
        setDomainCodes(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value5(JSON value) {
        setMeaningImages(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value6(JSON value) {
        setMediaFiles(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value7(String[] value) {
        setSemanticTypes(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value8(String[] value) {
        setLearnerComments(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value9(JSON value) {
        setNotes(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord value10(JSON value) {
        setDefinitions(value);
        return this;
    }

    @Override
    public MviewWwMeaningRecord values(Long value1, LocalDateTime value2, LocalDateTime value3, JSON value4, JSON value5, JSON value6, String[] value7, String[] value8, JSON value9, JSON value10) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached MviewWwMeaningRecord
     */
    public MviewWwMeaningRecord() {
        super(MviewWwMeaning.MVIEW_WW_MEANING);
    }

    /**
     * Create a detached, initialised MviewWwMeaningRecord
     */
    public MviewWwMeaningRecord(Long meaningId, LocalDateTime manualEventOn, LocalDateTime lastApproveOrEditEventOn, JSON domainCodes, JSON meaningImages, JSON mediaFiles, String[] semanticTypes, String[] learnerComments, JSON notes, JSON definitions) {
        super(MviewWwMeaning.MVIEW_WW_MEANING);

        setMeaningId(meaningId);
        setManualEventOn(manualEventOn);
        setLastApproveOrEditEventOn(lastApproveOrEditEventOn);
        setDomainCodes(domainCodes);
        setMeaningImages(meaningImages);
        setMediaFiles(mediaFiles);
        setSemanticTypes(semanticTypes);
        setLearnerComments(learnerComments);
        setNotes(notes);
        setDefinitions(definitions);
    }
}
