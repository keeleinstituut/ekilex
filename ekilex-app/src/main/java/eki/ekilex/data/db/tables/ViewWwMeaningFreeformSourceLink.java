...
public class ViewWwMeaningFreeformSourceLink extends TableImpl<ViewWwMeaningFreeformSourceLinkRecord> {
    ...
    private ViewWwMeaningFreeformSourceLink(Name alias, Table<ViewWwMeaningFreeformSourceLinkRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_ww_meaning_freeform_source_link\" as  SELECT meaning_id,\n    json_agg(ROW('FREEFORM'::character varying(100), freeform_id, source_link_id, type, name, order_by, source_id, source_name, source_value, source_value_prese, is_source_public)::type_source_link ORDER BY freeform_id, order_by) AS source_links\n   FROM ( SELECT mff.meaning_id,\n            mff.freeform_id,\n            ffsl_1.id AS source_link_id,\n            ffsl_1.type,\n            ffsl_1.name,\n            ffsl_1.value,\n            ffsl_1.order_by,\n            s.id AS source_id,\n            s.name AS source_name,\n            s.value AS source_value,\n            s.value_prese AS source_value_prese,\n            s.is_public AS is_source_public\n           FROM lexeme l,\n            dataset ds,\n            meaning_freeform mff,\n            freeform_source_link ffsl_1,\n            source s\n          WHERE ((l.is_public = true) AND (l.meaning_id = mff.meaning_id) AND (ffsl_1.freeform_id = mff.freeform_id) AND (ffsl_1.source_id = s.id) AND ((ds.code)::text = (l.dataset_code)::text) AND (ds.is_public = true))\n          GROUP BY mff.meaning_id, mff.freeform_id, ffsl_1.id, s.id) ffsl\n  GROUP BY meaning_id\n  ORDER BY meaning_id;"));
    }
