...
public class ViewWwWordEtymSourceLink extends TableImpl<ViewWwWordEtymSourceLinkRecord> {
     ...
     */
    public final TableField<ViewWwWordEtymSourceLinkRecord, JSON> SOURCE_LINKS = createField(DSL.name("source_links"), SQLDataType.JSON, this, "");
    ...
    private ViewWwWordEtymSourceLink(Name alias, Table<ViewWwWordEtymSourceLinkRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_ww_word_etym_source_link\" as  SELECT we.word_id,\n    json_agg(ROW('WORD_ETYM'::character varying(100), wesl.word_etym_id, wesl.id, wesl.type, wesl.name, wesl.order_by, s.id, s.name, s.value, s.value_prese, s.is_public)::type_source_link ORDER BY we.id, wesl.order_by) AS source_links\n   FROM word_etymology we,\n    word_etymology_source_link wesl,\n    source s\n  WHERE ((wesl.word_etym_id = we.id) AND (wesl.source_id = s.id) AND (EXISTS ( SELECT l.id\n           FROM lexeme l,\n            dataset ds\n          WHERE ((l.word_id = we.word_id) AND (l.is_public = true) AND ((ds.code)::text = (l.dataset_code)::text) AND (ds.is_public = true)))))\n  GROUP BY we.word_id\n  ORDER BY we.word_id;"));
    }
