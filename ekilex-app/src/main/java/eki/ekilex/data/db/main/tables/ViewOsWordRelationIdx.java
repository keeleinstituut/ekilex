...
public class ViewOsWordRelationIdx extends TableImpl<ViewOsWordRelationIdxRecord> {
    ...
    /**
     * The column <code>public.view_os_word_relation_idx.word_value</code>.
     */
    public final TableField<ViewOsWordRelationIdxRecord, String> WORD_VALUE = createField(DSL.name("word_value"), SQLDataType.CLOB, this, "");
    ...
    /**
     * The column <code>public.view_os_word_relation_idx.related_word_value</code>.
     */
    public final TableField<ViewOsWordRelationIdxRecord, String> RELATED_WORD_VALUE = createField(DSL.name("related_word_value"), SQLDataType.CLOB, this, "");

    /**
     * The column <code>public.view_os_word_relation_idx.related_word_value_as_word</code>.
     */
    public final TableField<ViewOsWordRelationIdxRecord, String> RELATED_WORD_VALUE_AS_WORD = createField(DSL.name("related_word_value_as_word"), SQLDataType.CLOB, this, "");
    ...
    private ViewOsWordRelationIdx(Name alias, Table<ViewOsWordRelationIdxRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_os_word_relation_idx\" as  SELECT w1.id AS word_id,\n    w1.value AS word_value,\n    wr.id AS word_relation_id,\n    wr.word_rel_type_code,\n    wr.order_by,\n    w2.id AS related_word_id,\n    w2.value AS related_word_value,\n    w2.value_as_word AS related_word_value_as_word\n   FROM word w1,\n    word w2,\n    word_relation wr\n  WHERE ((wr.word1_id = w1.id) AND (wr.word2_id = w2.id) AND (w1.lang = 'est'::bpchar) AND (w2.lang = 'est'::bpchar) AND (w1.is_public = true) AND (w2.is_public = true) AND ((wr.word_rel_type_code)::text = ANY (ARRAY[('ls-järelosaga'::character varying)::text, ('ls-esiosaga'::character varying)::text, ('deriv'::character varying)::text, ('komp'::character varying)::text, ('superl'::character varying)::text, ('posit'::character varying)::text])) AND (EXISTS ( SELECT 1\n           FROM lexeme l\n          WHERE ((l.word_id = w1.id) AND (l.is_public = true) AND (l.is_word = true) AND ((l.dataset_code)::text = 'eki'::text) AND (EXISTS ( SELECT 1\n                   FROM publishing p\n                  WHERE (((p.target_name)::text = 'ww_os'::text) AND ((p.entity_name)::text = 'lexeme'::text) AND (p.entity_id = l.id))))))) AND ((EXISTS ( SELECT 1\n           FROM publishing p\n          WHERE (((p.target_name)::text = 'ww_os'::text) AND ((p.entity_name)::text = 'word_relation'::text) AND (p.entity_id = wr.id)))) OR (((wr.word_rel_type_code)::text = ANY (ARRAY[('ls-järelosaga'::character varying)::text, ('ls-esiosaga'::character varying)::text])) AND (EXISTS ( SELECT 1\n           FROM word_relation wr2\n          WHERE ((wr2.word2_id = w2.id) AND (wr2.id <> wr.id) AND ((wr2.word_rel_type_code)::text = ANY (ARRAY[('ls-järelosaga'::character varying)::text, ('ls-esiosaga'::character varying)::text])) AND (EXISTS ( SELECT 1\n                   FROM lexeme l\n                  WHERE ((l.word_id = wr2.word1_id) AND (l.is_public = true) AND (l.is_word = true) AND ((l.dataset_code)::text = 'eki'::text) AND (EXISTS ( SELECT 1\n                           FROM publishing p\n                          WHERE (((p.target_name)::text = 'ww_os'::text) AND ((p.entity_name)::text = 'lexeme'::text) AND (p.entity_id = l.id))))))) AND (EXISTS ( SELECT 1\n                   FROM publishing p\n                  WHERE (((p.target_name)::text = 'ww_os'::text) AND ((p.entity_name)::text = 'word_relation'::text) AND (p.entity_id = wr2.id))))))))))\n  ORDER BY w1.id, wr.id;"));
    }
