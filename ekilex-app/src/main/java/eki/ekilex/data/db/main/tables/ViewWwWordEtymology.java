...
public class ViewWwWordEtymology extends TableImpl<ViewWwWordEtymologyRecord> {
    ...
    /**
     * The column <code>public.view_ww_word_etymology.word_etym_word_value</code>.
     */
    public final TableField<ViewWwWordEtymologyRecord, String> WORD_ETYM_WORD_VALUE = createField(DSL.name("word_etym_word_value"), SQLDataType.CLOB, this, "");

    /**
     * The column <code>public.view_ww_word_etymology.word_etym_word_lang</code>.
     */
    public final TableField<ViewWwWordEtymologyRecord, String> WORD_ETYM_WORD_LANG = createField(DSL.name("word_etym_word_lang"), SQLDataType.CHAR(3), this, "");
    ...
    /**
     * The column <code>public.view_ww_word_etymology.word_etym_meaning_word_values</code>.
     */
    public final TableField<ViewWwWordEtymologyRecord, String[]> WORD_ETYM_MEANING_WORD_VALUES = createField(DSL.name("word_etym_meaning_word_values"), SQLDataType.CLOB.getArrayDataType(), this, "");

    /**
     * The column <code>public.view_ww_word_etymology.word_etym_relations</code>.
     */
    public final TableField<ViewWwWordEtymologyRecord, JSON> WORD_ETYM_RELATIONS = createField(DSL.name("word_etym_relations"), SQLDataType.JSON, this, "");

    /**
     * The column <code>public.view_ww_word_etymology.source_links</code>.
     */
    public final TableField<ViewWwWordEtymologyRecord, JSON> SOURCE_LINKS = createField(DSL.name("source_links"), SQLDataType.JSON, this, "");
    ...
    private ViewWwWordEtymology(Name alias, Table<ViewWwWordEtymologyRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_ww_word_etymology\" as  WITH RECURSIVE word_etym_recursion(word_id, word_etym_word_id, word_etym_id, related_word_id, related_word_ids) AS (\n        ( SELECT we_1.word_id,\n            we_1.word_id AS word_etym_word_id,\n            we_1.id AS word_etym_id,\n            wer.related_word_id,\n            ARRAY[we_1.word_id] AS related_word_ids\n           FROM (word_etymology we_1\n             LEFT JOIN word_etymology_relation wer ON (((wer.word_etym_id = we_1.id) AND (wer.related_word_id <> we_1.word_id))))\n          WHERE (EXISTS ( SELECT l.id\n                   FROM lexeme l,\n                    dataset ds\n                  WHERE ((l.word_id = we_1.word_id) AND (l.is_public = true) AND ((ds.code)::text = (l.dataset_code)::text) AND ((ds.type)::text = 'LEX'::text) AND (EXISTS ( SELECT 1\n                           FROM publishing p\n                          WHERE (((p.entity_name)::text = 'lexeme'::text) AND (p.entity_id = l.id)))))))\n          ORDER BY we_1.order_by, wer.order_by)\n        UNION ALL\n        ( SELECT rec_1.word_id,\n            we_1.word_id AS word_etym_word_id,\n            we_1.id AS word_etym_id,\n            wer.related_word_id,\n            (rec_1.related_word_ids || we_1.word_id) AS related_word_ids\n           FROM ((word_etym_recursion rec_1\n             JOIN word_etymology we_1 ON ((we_1.word_id = rec_1.related_word_id)))\n             LEFT JOIN word_etymology_relation wer ON (((wer.word_etym_id = we_1.id) AND (wer.related_word_id <> we_1.word_id))))\n          WHERE (rec_1.related_word_id <> ANY (rec_1.related_word_ids))\n          ORDER BY we_1.order_by, wer.order_by)\n        )\n SELECT rec.word_id,\n    rec.word_etym_id,\n    rec.word_etym_word_id,\n    w.value AS word_etym_word_value,\n    w.lang AS word_etym_word_lang,\n    we.etymology_type_code,\n    we.etymology_year,\n    we.comment_prese AS word_etym_comment,\n    we.is_questionable AS word_etym_is_questionable,\n    we.order_by AS word_etym_order_by,\n    ( SELECT array_agg(w2.value) AS array_agg\n           FROM lexeme l1,\n            meaning m,\n            lexeme l2,\n            word w2\n          WHERE ((l1.meaning_id = m.id) AND (l2.meaning_id = m.id) AND (l1.word_id <> l2.word_id) AND (l1.is_public = true) AND ((l2.dataset_code)::text = 'ety'::text) AND (l2.is_public = true) AND (l2.word_id = w2.id) AND (w2.lang = 'est'::bpchar) AND (l1.word_id = rec.word_etym_word_id))) AS word_etym_meaning_word_values,\n    ( SELECT json_agg(json_build_object('wordEtymRelId', wer.id, 'comment', wer.comment_prese, 'questionable', wer.is_questionable, 'compound', wer.is_compound, 'relatedWordId', wer.related_word_id) ORDER BY wer.order_by) AS json_agg\n           FROM word_etymology_relation wer\n          WHERE ((wer.word_etym_id = rec.word_etym_id) AND (wer.related_word_id <> rec.word_id))) AS word_etym_relations,\n    ( SELECT json_agg(json_build_object('sourceLinkId', wesl.id, 'sourceLinkName', wesl.name, 'orderBy', wesl.order_by, 'sourceId', s.id, 'sourceName', s.name, 'sourceValue', s.value, 'sourceValuePrese', s.value_prese, 'sourcePublic', s.is_public) ORDER BY wesl.word_etym_id, wesl.order_by) AS json_agg\n           FROM word_etymology_source_link wesl,\n            source s\n          WHERE ((wesl.source_id = s.id) AND (wesl.word_etym_id = rec.word_etym_id))) AS source_links\n   FROM ((word_etym_recursion rec\n     JOIN word_etymology we ON ((we.id = rec.word_etym_id)))\n     JOIN ( SELECT w_1.id,\n            w_1.lang,\n            w_1.value\n           FROM word w_1\n          WHERE (EXISTS ( SELECT l.id\n                   FROM lexeme l,\n                    dataset ds\n                  WHERE ((l.word_id = w_1.id) AND (l.is_public = true) AND ((ds.code)::text = (l.dataset_code)::text) AND ((ds.type)::text = 'LEX'::text) AND ((l.dataset_code)::text <> 'ety'::text) AND ((EXISTS ( SELECT 1\n                           FROM publishing p\n                          WHERE (((p.entity_name)::text = 'lexeme'::text) AND (p.entity_id = l.id)))) OR ((l.dataset_code)::text <> 'eki'::text)))))\n          GROUP BY w_1.id) w ON ((w.id = rec.word_etym_word_id)))\n  ORDER BY rec.word_id, we.order_by;"));
    }
