...
public class ViewWwLexemeSourceLink extends TableImpl<ViewWwLexemeSourceLinkRecord> {
    ...
    private ViewWwLexemeSourceLink(Name alias, Table<ViewWwLexemeSourceLinkRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_ww_lexeme_source_link\" as  SELECT l.id AS lexeme_id,\n    json_agg(ROW('LEXEME'::character varying(100), l.id, lsl.id, lsl.type, lsl.name, lsl.order_by, s.id, s.name, s.value, s.value_prese, s.is_public)::type_source_link ORDER BY l.id, lsl.order_by) AS source_links\n   FROM lexeme l,\n    dataset ds,\n    lexeme_source_link lsl,\n    source s\n  WHERE ((l.is_public = true) AND (lsl.lexeme_id = l.id) AND (lsl.source_id = s.id) AND ((ds.code)::text = (l.dataset_code)::text) AND (ds.is_public = true))\n  GROUP BY l.id\n  ORDER BY l.id;"));
    }
