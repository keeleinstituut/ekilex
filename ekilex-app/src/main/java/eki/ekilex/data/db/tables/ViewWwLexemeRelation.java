...
public class ViewWwLexemeRelation extends TableImpl<ViewWwLexemeRelationRecord> {
    ...
    private static final long serialVersionUID = -1842762622;
     ...
     */
    public final TableField<ViewWwLexemeRelationRecord, Long> LEXEME_ID = createField(DSL.name("lexeme_id"), org.jooq.impl.SQLDataType.BIGINT, this, "");
    ...
    private ViewWwLexemeRelation(Name alias, Table<ViewWwLexemeRelationRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_ww_lexeme_relation\" as  SELECT r.lexeme1_id AS lexeme_id,\n    array_agg(ROW(l2.lexeme_id, w2.word_id, w2.word, w2.word_prese, w2.homonym_nr, w2.lang, (w2.word_type_codes)::character varying(100)[], l2.complexity, r.lex_rel_type_code)::type_lexeme_relation ORDER BY r.order_by) AS related_lexemes\n   FROM ((lex_relation r\n     JOIN ( SELECT l.id AS lexeme_id,\n            l.word_id,\n            l.complexity\n           FROM lexeme l,\n            dataset lds\n          WHERE (((l.type)::text = 'PRIMARY'::text) AND ((l.process_state_code)::text = 'avalik'::text) AND ((lds.code)::text = (l.dataset_code)::text) AND (lds.is_public = true))) l2 ON ((l2.lexeme_id = r.lexeme2_id)))\n     JOIN ( SELECT w.id AS word_id,\n            (array_agg(DISTINCT f.value))[1] AS word,\n            (array_agg(DISTINCT f.value_prese))[1] AS word_prese,\n            w.homonym_nr,\n            w.lang,\n            array_agg(wt.word_type_code ORDER BY wt.order_by) AS word_type_codes\n           FROM (((word w\n             JOIN paradigm p ON ((p.word_id = w.id)))\n             JOIN form f ON (((f.paradigm_id = p.id) AND ((f.mode)::text = 'WORD'::text))))\n             LEFT JOIN word_word_type wt ON ((wt.word_id = w.id)))\n          GROUP BY w.id) w2 ON ((w2.word_id = l2.word_id)))\n  WHERE (EXISTS ( SELECT l1.id\n           FROM lexeme l1,\n            dataset l1ds\n          WHERE ((l1.id = r.lexeme1_id) AND ((l1.type)::text = 'PRIMARY'::text) AND ((l1.process_state_code)::text = 'avalik'::text) AND ((l1ds.code)::text = (l1.dataset_code)::text) AND (l1ds.is_public = true))))\n  GROUP BY r.lexeme1_id;"));
    }
