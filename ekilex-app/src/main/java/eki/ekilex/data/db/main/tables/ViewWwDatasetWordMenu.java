...
public class ViewWwDatasetWordMenu extends TableImpl<ViewWwDatasetWordMenuRecord> {
    ...
    private ViewWwDatasetWordMenu(Name alias, Table<ViewWwDatasetWordMenuRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_ww_dataset_word_menu\" as  SELECT dataset_code,\n    first_letter,\n    array_agg(value ORDER BY value) AS word_values\n   FROM ( SELECT upper(\"left\"(w.value, 1)) AS first_letter,\n            w.value,\n            l.dataset_code\n           FROM word w,\n            lexeme l,\n            dataset ds\n          WHERE ((w.value <> ''::text) AND (w.lang = 'est'::bpchar) AND (w.is_public = true) AND (l.word_id = w.id) AND (l.is_public = true) AND (l.is_word = true) AND ((l.dataset_code)::text = (ds.code)::text) AND (ds.is_public = true) AND ((ds.code)::text <> ALL (ARRAY[('ety'::character varying)::text, ('eki'::character varying)::text])))) dsw\n  GROUP BY dataset_code, first_letter\n  ORDER BY dataset_code, first_letter;"));
    }
