...
public class ViewWwMeaningRelation extends TableImpl<ViewWwMeaningRelationRecord> {
    ...
    private static final long serialVersionUID = 115270097;
    ...
    private ViewWwMeaningRelation(Name alias, Table<ViewWwMeaningRelationRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"view_ww_meaning_relation\" as  SELECT r.m1_id AS meaning_id,\n    array_agg(ROW(r.m2_id, r.word_id, r.word, r.word_prese, r.homonym_nr, r.word_lang, r.aspect_code, (r.word_type_codes)::character varying(100)[], r.complexity, (r.lex_value_state_codes)::character varying(100)[], (r.lex_register_codes)::character varying(100)[], r.lex_government_values, r.meaning_rel_type_code)::type_meaning_relation ORDER BY r.order_by, r.lex_order_by) AS related_meanings\n   FROM ( SELECT mr.meaning1_id AS m1_id,\n            mr.meaning2_id AS m2_id,\n            w.id AS word_id,\n            (array_agg(DISTINCT f.value))[1] AS word,\n            (array_agg(DISTINCT f.value_prese))[1] AS word_prese,\n            w.homonym_nr,\n            w.lang AS word_lang,\n            w.aspect_code,\n            l.complexity,\n            ( SELECT array_agg(wt.word_type_code) AS array_agg\n                   FROM word_word_type wt\n                  WHERE (wt.word_id = w.id)\n                  GROUP BY w.id) AS word_type_codes,\n            ( SELECT array_agg(DISTINCT l_1.value_state_code) AS array_agg\n                   FROM lexeme l_1,\n                    dataset l_ds_1\n                  WHERE ((l_1.meaning_id = m.id) AND (l_1.word_id = w.id) AND ((l_1.type)::text = 'PRIMARY'::text) AND ((l_1.process_state_code)::text = 'avalik'::text) AND ((l_ds_1.code)::text = (l_1.dataset_code)::text) AND (l_ds_1.is_public = true) AND (l_1.value_state_code IS NOT NULL))\n                  GROUP BY l_1.word_id, l_1.meaning_id) AS lex_value_state_codes,\n            ( SELECT array_agg(DISTINCT lr.register_code) AS array_agg\n                   FROM lexeme_register lr,\n                    lexeme l_1,\n                    dataset l_ds_1\n                  WHERE ((l_1.meaning_id = m.id) AND (l_1.word_id = w.id) AND (lr.lexeme_id = l_1.id) AND ((l_1.type)::text = 'PRIMARY'::text) AND ((l_1.process_state_code)::text = 'avalik'::text) AND ((l_ds_1.code)::text = (l_1.dataset_code)::text) AND (l_ds_1.is_public = true))\n                  GROUP BY l_1.word_id, l_1.meaning_id) AS lex_register_codes,\n            ( SELECT array_agg(ff.value_text) AS array_agg\n                   FROM freeform ff,\n                    lexeme_freeform lff,\n                    lexeme l_1,\n                    dataset l_ds_1\n                  WHERE ((l_1.meaning_id = m.id) AND (l_1.word_id = w.id) AND ((l_1.type)::text = 'PRIMARY'::text) AND ((l_1.process_state_code)::text = 'avalik'::text) AND ((l_ds_1.code)::text = (l_1.dataset_code)::text) AND (l_ds_1.is_public = true) AND (lff.lexeme_id = l_1.id) AND (ff.id = lff.freeform_id) AND ((ff.type)::text = 'GOVERNMENT'::text))\n                  GROUP BY l_1.word_id, l_1.meaning_id) AS lex_government_values,\n            l.order_by AS lex_order_by,\n            mr.meaning_rel_type_code,\n            mr.order_by\n           FROM ((((((meaning_relation mr\n             JOIN meaning m ON ((m.id = mr.meaning2_id)))\n             JOIN lexeme l ON (((l.meaning_id = m.id) AND ((l.type)::text = 'PRIMARY'::text) AND ((l.process_state_code)::text = 'avalik'::text))))\n             JOIN word w ON ((w.id = l.word_id)))\n             JOIN paradigm p ON ((p.word_id = w.id)))\n             JOIN form f ON (((f.paradigm_id = p.id) AND ((f.mode)::text = 'WORD'::text))))\n             JOIN dataset l_ds ON ((((l_ds.code)::text = (l.dataset_code)::text) AND (l_ds.is_public = true))))\n          WHERE (EXISTS ( SELECT lex.id\n                   FROM lexeme lex,\n                    dataset lex_ds\n                  WHERE ((lex.meaning_id = mr.meaning1_id) AND ((lex.type)::text = 'PRIMARY'::text) AND ((lex.process_state_code)::text = 'avalik'::text) AND ((lex_ds.code)::text = (lex.dataset_code)::text) AND (lex_ds.is_public = true))))\n          GROUP BY m.id, mr.id, w.id, l.id) r\n  GROUP BY r.m1_id;"));
    }
