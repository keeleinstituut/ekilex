/*
 * This file is generated by jOOQ.
 */
package eki.ekilex.data.db.main.tables.records;


import eki.ekilex.data.db.main.tables.WordTag;

import java.sql.Timestamp;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record4;
import org.jooq.Row4;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class WordTagRecord extends UpdatableRecordImpl<WordTagRecord> implements Record4<Long, Long, String, Timestamp> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>public.word_tag.id</code>.
     */
    public void setId(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.word_tag.id</code>.
     */
    public Long getId() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.word_tag.word_id</code>.
     */
    public void setWordId(Long value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.word_tag.word_id</code>.
     */
    public Long getWordId() {
        return (Long) get(1);
    }

    /**
     * Setter for <code>public.word_tag.tag_name</code>.
     */
    public void setTagName(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.word_tag.tag_name</code>.
     */
    public String getTagName() {
        return (String) get(2);
    }

    /**
     * Setter for <code>public.word_tag.created_on</code>.
     */
    public void setCreatedOn(Timestamp value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.word_tag.created_on</code>.
     */
    public Timestamp getCreatedOn() {
        return (Timestamp) get(3);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Long> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record4 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row4<Long, Long, String, Timestamp> fieldsRow() {
        return (Row4) super.fieldsRow();
    }

    @Override
    public Row4<Long, Long, String, Timestamp> valuesRow() {
        return (Row4) super.valuesRow();
    }

    @Override
    public Field<Long> field1() {
        return WordTag.WORD_TAG.ID;
    }

    @Override
    public Field<Long> field2() {
        return WordTag.WORD_TAG.WORD_ID;
    }

    @Override
    public Field<String> field3() {
        return WordTag.WORD_TAG.TAG_NAME;
    }

    @Override
    public Field<Timestamp> field4() {
        return WordTag.WORD_TAG.CREATED_ON;
    }

    @Override
    public Long component1() {
        return getId();
    }

    @Override
    public Long component2() {
        return getWordId();
    }

    @Override
    public String component3() {
        return getTagName();
    }

    @Override
    public Timestamp component4() {
        return getCreatedOn();
    }

    @Override
    public Long value1() {
        return getId();
    }

    @Override
    public Long value2() {
        return getWordId();
    }

    @Override
    public String value3() {
        return getTagName();
    }

    @Override
    public Timestamp value4() {
        return getCreatedOn();
    }

    @Override
    public WordTagRecord value1(Long value) {
        setId(value);
        return this;
    }

    @Override
    public WordTagRecord value2(Long value) {
        setWordId(value);
        return this;
    }

    @Override
    public WordTagRecord value3(String value) {
        setTagName(value);
        return this;
    }

    @Override
    public WordTagRecord value4(Timestamp value) {
        setCreatedOn(value);
        return this;
    }

    @Override
    public WordTagRecord values(Long value1, Long value2, String value3, Timestamp value4) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached WordTagRecord
     */
    public WordTagRecord() {
        super(WordTag.WORD_TAG);
    }

    /**
     * Create a detached, initialised WordTagRecord
     */
    public WordTagRecord(Long id, Long wordId, String tagName, Timestamp createdOn) {
        super(WordTag.WORD_TAG);

        setId(id);
        setWordId(wordId);
        setTagName(tagName);
        setCreatedOn(createdOn);
    }
}
