...
public class SmtpTransport extends Transport {
	...
	protected SmtpTransport(Session session, URLName urlname, String name, boolean isSSL) {
		...
		logger = new MailLogger(this.getClass(), "DEBUG SMTP", session.getDebug(), session.getDebugOut());
		traceLogger = logger.getSubLogger("protocol", null);
		noauthdebug = !PropUtil.getBooleanProperty(props, "mail.debug.auth", false);
		debugusername = PropUtil.getBooleanProperty(props, "mail.debug.auth.username", true);
		debugpassword = PropUtil.getBooleanProperty(props, "mail.debug.auth.password", false);
		if (urlname != null) {
		...
		quitWait = PropUtil.getBooleanProperty(props, "mail." + name + ".quitwait", true);

		// setting mail.smtp.quitonsessionreject to false causes us to directly
		// close the socket without sending a QUIT command
		quitOnSessionReject = PropUtil.getBooleanProperty(props, "mail." + name + ".quitonsessionreject", false);

		// mail.smtp.reportsuccess causes us to throw an exception on success
		reportSuccess = PropUtil.getBooleanProperty(props, "mail." + name + ".reportsuccess", false);

		// mail.smtp.starttls.enable enables use of STARTTLS command
		useStartTLS = PropUtil.getBooleanProperty(props, "mail." + name + ".starttls.enable", false);

		// mail.smtp.starttls.required requires use of STARTTLS command
		requireStartTLS = PropUtil.getBooleanProperty(props, "mail." + name + ".starttls.required", false);

		// mail.smtp.userset causes us to use RSET instead of NOOP
		// for isConnected
		useRset = PropUtil.getBooleanProperty(props, "mail." + name + ".userset", false);

		// mail.smtp.noop.strict requires 250 response to indicate success
		noopStrict = PropUtil.getBooleanProperty(props, "mail." + name + ".noop.strict", true);

		// check if SASL is enabled
		enableSASL = PropUtil.getBooleanProperty(props, "mail." + name + ".sasl.enable", false);
		if (enableSASL) {
			logger.config("enable SASL");
		}
		useCanonicalHostName = PropUtil.getBooleanProperty(props, "mail." + name + ".sasl.usecanonicalhostname", false);
		if (useCanonicalHostName) {
			logger.config("use canonical host name");
		}

		allowutf8 = PropUtil.getBooleanProperty(props, "mail.mime.allowutf8", false);
		if (allowutf8) {
			logger.config("allow UTF-8");
		}

		chunkSize = PropUtil.getIntProperty(props, "mail." + name + ".chunksize", -1);
		...
		for (Authenticator element : a) {
			authenticators.put(element.getMechanism(), element);
			sb.append(element.getMechanism()).append(' ');
		}
	...
	private boolean authenticate(String user, String passwd)
		...
		while (st.hasMoreTokens()) {
			...
			Authenticator authenticator = authenticators.get(m);
			...
			if (mechs == defaultAuthenticationMechanisms) {
				...
				boolean disabled = PropUtil.getBooleanProperty(session.getProperties(), dprop, !authenticator.enabled());
				if (disabled) {
					if (logger.isLoggable(Level.FINE)) {
			...
			return authenticator.authenticate(host, authzid, user, passwd);
		}
	...
	private abstract class Authenticator {
		...
		boolean authenticate(String host, String authzid, String user, String passwd) throws MessagingException {
			...
			try {
				...
				String initialResponse = getInitialResponse(host, authzid, user, passwd);
				if (noauthdebug && isTracing()) {
					logger.fine("AUTH " + mech + " command trace suppressed");
					suspendTracing();
				}
				String cmd = "AUTH " + mech + " " + initialResponse;
				resp = simpleCommand(cmd);
				...
				if (resp == 530) {
					...
					if (initialResponse != null) {
						resp = simpleCommand("AUTH " + mech + " " + initialResponse);
					} else {
		...
		abstract void doAuth(String host, String authzid, String user, String passwd) throws MessagingException, IOException;
	}
	...
	private class OAuth2Authenticator extends Authenticator {
		...
		String getInitialResponse(String host, String authzid, String user, String passwd) throws MessagingException, IOException {
			...
			*/
			...
			final String resp = "user=" + user + ctrlA + "auth=Bearer " + passwd + ctrlA + ctrlA;
			String encResp = Base64.getEncoder().encodeToString(resp.getBytes());
			return encResp;
		}
	...
	private boolean sasllogin(String[] allowed, String realm, String authzid,
		...
		if (allowed != null && allowed.length > 0) {
			...
			for (String element : allowed) {
				if (supportsAuthentication(element)) { // XXX - case must match
					v.add(element);
				}
			}
		} else {
	...
	public synchronized void sendMessage(Message message, Address[] addresses)
		...
		for (Address element : addresses) {
			if (!(element instanceof InternetAddress)) {
				throw new MessagingException(element +
		...
		for (int i = 0; i < addresses.length; i++) {
			...
			if (a.isGroup()) {
				...
				try {
					...
					if (ia != null) {
						for (InternetAddress element : ia) {
							groups.add(element);
						}
					} else {
						groups.add(a);
	...
	protected void rcptTo() throws MessagingException {
		...
		if (!sendPartial) {
			sendPartial = PropUtil.getBooleanProperty(session.getProperties(), "mail." + name + ".sendpartial", false);
		}
		if (sendPartial) {
		...
		for (Address element : addresses) {
			...
			sfex = null;
			InternetAddress ia = (InternetAddress) element;
		...
		if (sendFailed) {
			...
			int i = 0;
			for (InternetAddress element : valid) {
				validUnsentAddr[i++] = element;
			}
			for (InternetAddress element : validUnsent) {
				validUnsentAddr[i++] = element;
			}
	...
	private void openServer(String host, int port) throws MessagingException {
		...
		try {
			...
			if ((r = readServerResponse()) != 220) {
				...
				if (logger.isLoggable(Level.FINE)) {
					logger.fine("got bad greeting from host \"" + host + "\", port: " + port + ", response: " + failResponse);
				}
	...
	private void initStreams() throws IOException {
		boolean quote = PropUtil.getBooleanProperty(session.getProperties(), "mail.debug.quote", false);
