...
begin
  ...
  ll_row_div := ll_row_cnt / 10;
  raise info 'iterating over % lifecycle log records', ll_row_cnt;
  for ll_row in
    (select * from lifecycle_log order by id)
  loop
    select
    ...
    if ll_row_itr % ll_row_div = 0 then
      ll_row_prc := (ll_row_itr * 100) / ll_row_cnt;
      raise info 'currently @ % perc. (row % of %)', ll_row_prc, ll_row_itr, ll_row_cnt;
    end if;
    if ll_row.entity_prop = 'ASPECT' then
      al_id := create_activity_log_same_owner_and_entity(ll_row, '/word/aspectCode');
      ...
      if ll_row.entity_name = 'LEXEME' then
        ...
        and   ll.entity_id = ll_row.entity_id into tmp_txt;
        if tmp_txt = 'NOTE' then
          ll_row.entity_id := ll_dat.meaning_id;
          ll_row.entity_name := 'MEANING';
          al_id := create_activity_log_unknown_entity(ll_row, 'DEFINITION_NOTE_SOURCE_LINK', '/definitions/notes/sourceLinks/value');
          perform create_meaning_activity_log_and_bindings(ll_dat.meaning_id, al_id);
        elsif tmp_txt is not null then
          raise exception 'Unsupported freeform type % for the owner %', tmp_txt, ll_row.entity_name;
        end if;
      ...
      al_id := create_activity_log_same_owner_and_entity(ll_row, '/word/genderCode');
      perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
      ...
      if ll_row.event_type = 'ORDER_BY' then
        ...
        if ll_row.entity_name = 'WORD' then
          ...
          if tmp_txt is not null then
            ...
            al_id := create_activity_log_same_owner_and_entity(ll_row, '/word/wordValue');
            perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
          end if;
      ...
      al_id := create_activity_log_same_owner_and_entity(ll_row, '/word/lang');
      perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
      ...
      al_id := create_activity_log_unknown_entity(ll_row, ll_row.entity_prop, '/meaningWords/wordValue');
      perform create_lexeme_activity_log_and_bindings(ll_dat.lexeme_id, al_id);
      ...
      if ll_row.entity_name = 'LEXEME' then
        ...
        al_id := create_activity_log_unknown_entity(ll_row, 'WORD_NOTE', '/word/notes/valuePrese');
        perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
      elsif ll_row.entity_name = 'MEANING' then
        ...
        ll_row.entity_id := ll_dat.meaning_id;
        ll_row.entity_name := 'MEANING';
        al_id := create_activity_log_unknown_entity(ll_row, 'DEFINITION_NOTE', '/definitions/notes/valuePrese');
        perform create_meaning_activity_log_and_bindings(ll_dat.meaning_id, al_id);
      ...
      ll_row.entity_id := ll_dat.lexeme_id;
      ll_row.entity_name := 'LEXEME';
      al_id := create_activity_log_unknown_entity(ll_row, 'OD_USAGE_ALTERNATIVE', '/usages/odAlternatives/value');
      perform create_lexeme_activity_log_and_bindings(ll_dat.lexeme_id, al_id);
    elsif ll_row.entity_prop = 'OD_DEFINITION' then
      ll_row.entity_id := ll_dat.lexeme_id;
      ll_row.entity_name := 'LEXEME';
      al_id := create_activity_log_unknown_entity(ll_row, 'OD_USAGE_DEFINITION', '/usages/odDefinitions/value');
      perform create_lexeme_activity_log_and_bindings(ll_dat.lexeme_id, al_id);
      ...
      if ll_row.entity_name = 'LEXEME' then
        ...
        al_id := create_activity_log_unknown_entity(ll_row, 'OD_WORD_RECOMMENDATION', '/odLexemeRecommendations/valuePrese');
        perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
      end if;
      ...
      if ll_row.entity_name = 'LEXEME' then
        ...
        ll_row.entity_id := ll_dat.meaning_id;
        ll_row.entity_name := 'MEANING';
        al_id := create_activity_log_unknown_entity(ll_row, 'DEFINITION', '/definitions/orderBy');
        perform create_meaning_activity_log_and_bindings(ll_dat.meaning_id, al_id);
      end if;
      ...
      if ll_row.entity_name = 'LEXEME' then
        ...
        ll_row.entity_id := ll_dat.lexeme_id;
        ll_row.entity_name := 'LEXEME';
        al_id := create_activity_log_unknown_entity(ll_row, 'USAGE_SOURCE_LINK', '/usages/sourceLinks/value');
        perform create_lexeme_activity_log_and_bindings(ll_dat.lexeme_id, al_id);
      elsif ll_row.entity_name = 'DEFINITION' then
        ll_row.entity_id := ll_dat.meaning_id;
        ll_row.entity_name := 'MEANING';
        al_id := create_activity_log_unknown_entity(ll_row, 'DEFINITION_SOURCE_LINK', '/definitions/sourceLinks/value');
        perform create_meaning_activity_log_and_bindings(ll_dat.meaning_id, al_id);
      end if;
      ...
      if ll_row.entity_name = 'WORD_RELATION' then
        al_id := create_activity_log_for_owner_and_entity(ll_dat.word_id, 'WORD', ll_row, '/wordRelations/relationStatus');
        perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
      end if;
      ...
      if ll_row.entity_name = 'ATTRIBUTE_FREEFORM' then
        ...
        ll_row.entity_id := ll_dat.lexeme_id;
        ll_row.entity_name := 'LEXEME';
        al_id := create_activity_log_unknown_entity(ll_row, 'NOTE', '/lexemeNoteLangGroups/notes/valuePrese');
        perform create_lexeme_activity_log_and_bindings(ll_dat.lexeme_id, al_id);
      elsif ll_row.entity_name = 'LEXEME_RELATION' then
        ...
        al_id := create_activity_log_for_owner_and_entity(ll_dat.word_id, 'WORD', ll_row, '/wordGroups/members/*');
        perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
      end if;
      ...
      al_id := create_activity_log_unknown_entity(ll_row, ll_row.entity_prop, '/word/wordTypeCodes');
      perform create_word_activity_log_and_bindings(ll_dat.word_id, al_id);
    end if;
  end loop;
end $$;
...
drop function create_activity_log_same_owner_and_entity(bigint,bigint);
drop function create_activity_log_for_owner_and_entity(bigint,bigint);
drop function create_activity_log_unknown_entity(bigint,bigint);
drop function conv_event_type_to_op(bigint,bigint);
drop type wlm_id;
