...
where word.lang = :lang
  ...
  and form.mode = 'WORD'
  and (
    exists(select lexeme.id
           from lexeme
                  left join dataset on lexeme.dataset_code = dataset.code
           where dataset.type = :datasetType
             and lexeme.word_id = word.id)
    or exists(select lexeme.id
              from lexeme
              where lexeme.dataset_code = :includedDatasetCode
                and lexeme.word_id = word.id)
  )
  and not exists(select lexeme.id
                 from lexeme
                 where lexeme.dataset_code = :excludedDatasetCode
                   and lexeme.word_id = word.id)
  and not exists(select word_word_type.word_type_code
                 from word_word_type
